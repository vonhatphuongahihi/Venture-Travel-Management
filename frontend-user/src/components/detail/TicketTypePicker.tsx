...
function TicketTypePicker({ userTicket, setUserTicket, ticketPrices, ticketTypes, totalPrice }) {
              ...
              <div>
                {userTicket?.priceCategories?.map((ut, idx) => {
                  const matchedPrice = ticketPrices.find(
                    (tp) => tp.ticketTypeId === t.ticketTypeId && tp.categoryId === ut.categoryId
                  );
                  return (
                    <div>
                      {ut.quantity > 0 && (
                        <div key={idx} className="flex justify-between items-center">
                          <p className="text-xs">
                            ...
                            {matchedPrice ? matchedPrice.price.toLocaleString("vi-VN") : 0}₫
                          </p>
                          <p className="text-sm">
                            {(ut.quantity * (matchedPrice?.price || 0)).toLocaleString("vi-VN")}₫
                          </p>
                        </div>
                      )}
                    </div>
