import { useProvinceForecast } from "@/hooks/useProvince";
import { Card } from "../ui/card";
import { Loader2, Cloud, Sun, CloudRain, CloudSnow, CloudLightning, CloudDrizzle, CloudFog } from "lucide-react";
...
const getWeatherIcon = (code: number) => {
  ...
  if (code === 1 || code === 2 || code === 3) return <Cloud className="w-8 h-8 text-gray-400" />; // Mainly clear, partly cloudy, and overcast
  if (code === 45 || code === 48) return <CloudFog className="w-8 h-8 text-gray-400" />; // Fog
  if (code >= 51 && code <= 55) return <CloudDrizzle className="w-8 h-8 text-blue-300" />; // Drizzle
  if (code >= 61 && code <= 67) return <CloudRain className="w-8 h-8 text-blue-500" />; // Rain
  if (code >= 71 && code <= 77) return <CloudSnow className="w-8 h-8 text-blue-200" />; // Snow
  if (code >= 80 && code <= 82) return <CloudRain className="w-8 h-8 text-blue-600" />; // Rain showers
  if (code >= 85 && code <= 86) return <CloudSnow className="w-8 h-8 text-blue-300" />; // Snow showers
...
const getWeatherDescription = (code: number) => {
  switch (code) {
    ...
    case 66: case 67: return "Mưa băng";
    case 71: case 73: case 75: return "Tuyết rơi";
    case 77: return "Tuyết hạt";
    case 80: case 81: case 82: return "Mưa rào";
    case 85: case 86: return "Tuyết rào";
    case 95: case 96: case 99: return "Dông";
    default: return "Không xác định";
  }
};

const WeatherForecast = ({ lat, long }: WeatherForecastProps) => {
  const { data: forecastData, isLoading } = useProvinceForecast(lat, long);

  const dailyForecast = useMemo(() => {
    if (!forecastData || !forecastData.daily) return [];

    const { time, weathercode, temperature_2m_max, temperature_2m_min, precipitation_sum } = forecastData.daily;

    return time.map((dateStr: string, index: number) => ({
      date: new Date(dateStr).toLocaleDateString('vi-VN', { weekday: 'short', day: 'numeric', month: 'numeric' }),
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        {dailyForecast.map((day: any, index: number) => (
          <Card key={index} className="p-4 flex flex-col items-center text-center hover:shadow-md transition-shadow">
            <span className="text-sm font-medium text-gray-500 mb-2">{day.date}</span>
            <div className="mb-2">
