    ...
    >
      ...
      <div className="container relative">
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-bold mb-4">TOP ĐIỂM ĐẾN</h2>
          <p className="text-muted-foreground text-lg max-w-3xl mx-auto">
            Sapa với núi rừng hùng vĩ, Đà Lạt mộng mơ, các thành phố nhộn nhịp
            và những di tích lịch sử - văn hóa đặc sắc
          </p>
        </div>
            ...
            <div className="flex justify-center">
              <div className="flex items-center gap-6 max-w-6xl overflow-visible">
                {destinations.map((destination, index) => (
                  <div
                    key={destination.id}
                    className={`relative flex-shrink-0 w-64 h-80 rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 shadow-xl shadow-primary/10 border border-white/10 backdrop-blur-lg ${getCardOpacity(
                      index,
                      currentIndex
                    )}`}
                    style={{
