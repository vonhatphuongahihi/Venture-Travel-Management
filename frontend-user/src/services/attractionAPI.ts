...
class AttractionAPI {
    ...
    static async getTopDestinations(limit: number = 5): Promise<TopDestination[]> {
        try {
            const response = await fetch(`${API_BASE_URL}/attractions/top?limit=${limit}`, {
                method: 'GET',
                headers: {
            ...
            if (result.success && result.data && result.data.destinations) {
                return result.data.destinations.map((destination: any) => ({
                    ...
                    image: destination.image || (destination.images && destination.images.length > 0 ? destination.images[0] : '/placeholder.svg'),
    ...
    static async getAttractions(params?: {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/attractions?${queryParams.toString()}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!response.ok) {
                ...
                return { attractions: [], pagination: { page: 1, limit: 10, total: 0, totalPages: 0 } };
            }
            ...
            if (result.success && result.data && result.data.attractions) {
                return {
                    attractions: result.data.attractions.map((attraction: any) => ({
                        ...
                        image: attraction.image || (attraction.images && attraction.images.length > 0 ? attraction.images[0] : '/placeholder.svg'),
