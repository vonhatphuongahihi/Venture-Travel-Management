...
class ProvinceAPI {
    ...
    try {
      ...
      if (result.success && result.data) {
        return result.data.map((province: any) => ({
          ...
          slug:
            province.slug ||
            province.id?.toLowerCase().replace(/\s+/g, "-") ||
            "",
          image:
            province.image ||
  ...
  static async getToursByProvince(
    provinceId: string,
    page: number = 1,
    limit: number = 10,
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/provinces/${provinceId}/tours?page=${page}&limit=${limit}&sortBy=${sortBy}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  static async getAttractionsByProvince(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/provinces/${provinceId}/attractions`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  static async getReviewsByProvince(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/provinces/${provinceId}/reviews?page=${page}&limit=${limit}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      const result = await response.json();
      if (result.success && result.data) {
        // Map server response to match ReviewCard expectations
        const mappedData = result.data.data.map((review: any) => ({
          ...review,
          createdAt: review.date, // Map date to createdAt
          rate: review.rating, // Map rating to rate
          tourId: review.tour?.id,
          tour: review.tour
            ? {
                ...review.tour,
                name: review.tour.title, // Map title to name
                images: review.tour.image ? [review.tour.image] : [], // Map image to images array
              }
            : undefined,
        }));
